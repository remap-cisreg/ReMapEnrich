...
#' @param main=paste("Significance, top", top, "categories"). Allows to choose the title of the plot.
...
#' @param minCircleSize=0.5 Allows to change the minimum size of circle for the plot.
...
EnrichmentDotPlot <- function(enrich, 
    ...
    maxOverlapsLim <- maxOverlaps - (maxOverlaps %% stepOverlaps) + stepOverlaps
    ...
    with(enrich,
         symbols(
            ...
            y       = 1:top,
            circles = (nb.overlaps - minOverlaps) / (maxOverlapsLim - minOverlapsLim) + minCircleSize,
            inches  = inches,
            bg      = rgb(colorFunction(((sig - minSigLim) / (maxSigLim - minSigLim))), maxColorValue = 255),
            fg      = stainCol,
            main    = main,
            xlab    = xlab,
            ylab    = "",
    ...
    cols = rgb(colorFunction( ((sig - minSigLim) / (maxSigLim - minSigLim))), maxColorValue = 255)
    for (i in 1:top) {
    ...
    plot(c(0,1),c(minSigLim, maxSigLim),type = 'n', axes = FALSE, ylab = "", xlab = sigTypeTitle)
    ...
    text(x = 0.02, y = yCircles, labels = round(seq(minOverlapsLim, maxOverlapsLim, l = 5)), adj = 0)
